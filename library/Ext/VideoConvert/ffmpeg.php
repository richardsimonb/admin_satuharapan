...
;
define("MP4Box_BINARY",get_binaries(array('type'=>'user','path'=>'MP4Box')));
define("FLVTool2_BINARY",get_binaries(array('type'=>'user','path'=>'flvtool2')));
define('FFMPEG_BINARY', get_binaries(array('type'=>'user','path'=>'ffmpeg')));
define('USE_MPLAYER',true);
define('FILES_DIR',$_SERVER['DOCUMENT_ROOT'].'/assets/clips');
...
define('FILES_URL',BASEURL.'/clips');
define('VIDEOS_URL',BASEURL.'/videos');
define('THUMBS_URL',BASEURL.'/thumbs');
define('ORIGINAL_URL',BASEURL.'/original');
define('TEMP_URL',BASEURL.'/temp');


class Ext_VideoConvert_ffmpeg 
{
	 ...
	 * Initiating Class
	 */
	function Ext_VideoConvert_ffmpeg($file)
	{

		$this->ffmpeg = '/usr/bin/ffmpeg';
		$this->mp4box = '/usr/local/bin/MP4Box';
		$this->flvtool2 = '/usr/bin/flvtool2';
		$this->flvtoolpp = '';
		$this->mplayerpath = '';
	...
	function convert($file=NULL)
	{
        ...
        */if($this->flvtool2  && file_exists($this->output_file)  && @filesize($this->output_file)>0)
		{
			$tmp_file = time().RandomString(5).'flvtool2_output.tmp';
	...
	function parse_format_info( $output ) {
		...
		if(!isset($audio_codec) || !isset($audio_rate))
		{
	...
	function Processit()
	{
		...
		{
			$use_crons = 'yes';
			if($use_crons=='yes' || !$this->set_conv_lock)
			{
