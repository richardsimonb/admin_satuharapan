...
class Ext_View_Smarty extends Zend_View_Abstract
{
    ...
    private function _loadSmarty()
    {
        ...
        $this->_smartyPaginate = new SmartyPaginate;
		...
		$globalMethod = new Ext_Global_Function;
		$this->_smarty->registerObject('gLobalMethod', $globalMethod);
        
        if (class_exists('repository_local', true)) {
            $localMethod = new repository_local;
    		$this->_smarty->registerObject('localMethod', $localMethod);
        }		
		
        $this->assign('this', $this);
    }
    /**
     * SETTING UP FOR SMARTY PAGINATION
     * ADD BY ADE OFIK HIDAYAT     
     */
     public function loadSmartyPagination(){
        //die("anton goreng, ade ganteng");
        return $this->_smartyPaginate;
     }     
}