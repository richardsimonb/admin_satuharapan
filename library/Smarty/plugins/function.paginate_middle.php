...
function smarty_function_paginate_middle($params, &$smarty) {
    ...
    while($_item <= $_total) {
        ...
        if($_item != $_curr_item) {
            ...
            $_ret .= '<li>'.$_link_prefix . '<a href="' . str_replace('&', '&amp;', $_this_url) . '"' . $_attrs . '>' . $_text . '</a>' . $_link_suffix.'</li>';
        } else {
            $_ret .= '<li class="active"><a href="javascript:void(0)">' . $_link_prefix . $_text . $_link_suffix . '</a></li>';
        }
        $_item += $_limit;
        $_page++;
        $_display_pages++;
