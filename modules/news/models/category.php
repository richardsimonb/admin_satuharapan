...
class category extends Ext_Redwhite_Db
{
    ...
    {
        $rows = $this->fetchAll("parent_category=".$parent_category . " AND hidden=0 AND deleted=0","sorting");
        if($rows){
            ...
            foreach($categories as  $category)
            {
                ...
                $subRows = $this->fetchAll("parent_category=".$category['uid'] . " AND hidden=0 AND deleted=0");
                if($subRows)
                {
