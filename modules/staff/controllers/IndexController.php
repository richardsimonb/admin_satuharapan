...
class staff_IndexController extends Ext_Controller_Action 
{
  ...
  public function listAction()
  {
    ...
    //  die("asd");        
	 ...
	 $numrows  = $this->feusers->getNumRows();
	 //die($numrows);
    ...
    if($this->getRequest()->isPost())
     {
        ...
        //$_POST['f']['name'] = $_POST['f']['first_name']." ".$_POST['f']['middle_name']." ".$_POST['f']['last_name'];
        $exist = $this->feusers->fetchRow("username ='". trim($_POST['f']['username'])."' OR email='".trim($_POST['f']['email'])."'");
        if ($exist)
        {
            $reply['status'] = "error";
            $reply['message'] = "Username sudah ada / e-mail sudah ada";
        } else {
